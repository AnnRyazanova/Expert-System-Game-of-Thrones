...
персонаж(король_ночи, летал_на_драконах).
персонаж(король_ночи, мог_бы_участвовать_в_олимпийских_играх).
персонаж(король_ночи, мертв).
...
все_факты :- получение_фактов_персонажи(Список, Длина), write('Факты о персонажах : \n'), печать(Список, 1), 
			 получение_фактов_оружие(Список2, Длина2), L is (Длина + 1), write('\n Оружие : \n'), печать(Список2, L),
			 получение_фактов_место_обитания(Список3, Длина3), L2 is (L + Длина2), write('\n Места присутствия : \n'), печать(Список3, L2),
			 получение_фактов_советники(Список4, _), L3 is (L2 + Длина3), write('\n Являются советниками : \n'), печать(Список4, L3).
...
поиск_персонажей_удовлетворяющих_факту(Индекс, Результат) :- поиск_по_индексу(Индекс, Факт), 
													   ...
													   объединить(Р1, Р2, Р3), удаление_дубликатов(Р3, Результат).  
													   

поиск_персонажей_по_всем_фактам([H], Результат) :- поиск_персонажей_удовлетворяющих_факту(H, Результат), !.													   													   
поиск_персонажей_по_всем_фактам([H|T], Результат) :- поиск_персонажей_удовлетворяющих_факту(H, Список),
													 поиск_персонажей_по_всем_фактам(T, Персонажи), пересечение(Персонажи, Список, Рез),
													 удаление_дубликатов(Рез, Результат).
													 
пересечение([], _, []).	
пересечение([H1 | T1], Список2, [H1 | T2]) :- входит(H1, Список2), !, пересечение(T1, Список2, T2).  
пересечение([ _ | T1], Список2, Список3) :-  пересечение(T1, Список2, Список3).  


												 

персонажи_по_всем_фактам(Список_фактов) :- поиск_персонажей_по_всем_фактам(Список_фактов, Персонажи), 
										   length(Персонажи, Длина), (Длина =\= 0), !,
										   печать(Персонажи, 1).
персонажи_по_всем_фактам(_) :- write('Нет персонажей, удовлетворяющих следующим фактам'), !.
